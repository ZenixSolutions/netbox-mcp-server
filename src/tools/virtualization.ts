...
const clusterCreate = {
  ...
  scope_type: z
    ...
    .optional()
    .describe(
      "Scope object type (NetBox 4.2+ replaced 'site' with scope); set with scope_id.",
    ),
...
const clusterUpdate = {
  ...
  status: z.enum(CLUSTER_STATUS).optional(),
  scope_type: z.enum(CLUSTER_SCOPES).nullable().optional(),
...
const vmTypeCreate = {
  name: z.string().min(1).max(100).describe("Name (required)."),
  slug: z.string().min(1).max(100).regex(SLUG).describe("URL slug (required)."),
  default_platform: z.number().int().optional(),
  default_vcpus: z.number().optional(),
  default_memory: z.number().int().optional().describe("Default memory (MB)."),
  description: z.string().max(200).optional(),
  comments: z.string().optional(),
  tags: TagSlugsSchema,
  custom_fields: CustomFieldsSchema,
...
const vmTypeUpdate = {
  ...
  default_vcpus: z.number().nullable().optional(),
  default_memory: z.number().int().nullable().optional(),
  description: z.string().max(200).optional(),
  comments: z.string().optional(),
  tags: TagSlugsSchema,
  custom_fields: CustomFieldsSchema,
...
const vmCreate = {
  ...
  cluster: z
    ...
    .int()
    .optional()
    .describe("Cluster id (VM needs a cluster or a device)."),
  device_id: z
    .number()
    .int()
    .optional()
    .describe(
      "Host device id (alternative to cluster). Exposed as device_id (not device) for the bridge; mapped to the API field device on write.",
    ),
  ...
  memory: z.number().int().optional().describe("Memory (MB)."),
  disk: z.number().int().optional().describe("Disk (MB)."),
  primary_ip4: z.number().int().optional(),
  primary_ip6: z.number().int().optional(),
...
const vmUpdate = {
  ...
  status: z.enum(VM_STATUS).optional(),
...
const vmIfaceCreate = {
  ...
  untagged_vlan: z.number().int().optional(),
...
const vdiskCreate = {
  virtual_machine: z.number().int().describe("Virtual machine id (required)."),
  name: z.string().min(1).max(64).describe("Disk name (required)."),
  size: z.number().int().describe("Size in MB (required)."),
  description: z.string().max(200).optional(),
  tags: TagSlugsSchema,
  custom_fields: CustomFieldsSchema,
};
...
export function registerVirtualization(server: McpServer): void {
  ...
  crud(
    ...
    vmCreate,
    vmUpdate,
    ["name", "status", "cluster", "role", "vcpus", "memory"],
  );
  crud(
    ...
    { virtual_machine_id: z.number().int().optional(), name: z.string().optional() },
    vmIfaceCreate,
    vmIfaceUpdate,
    ["name", "virtual_machine", "enabled", "mtu"],
  );
  crud(
