...
const rirCreate = {
  ...
  is_private: z
    .boolean()
    .optional()
    .describe("True for RFC 1918 / private space authorities."),
...
export function registerIpamOrg(server: McpServer): void {
  ...
  crud(
    ...
    {
      asn: z.number().int().optional(),
      rir_id: z.number().int().optional(),
      tenant_id: z.number().int().optional(),
    },
    asnCreate,
    asnUpdate,
    ["asn", "rir", "tenant", "description"],
  );
  crud(
    "ipam/asn-ranges",
    "asn_range",
    "asn_ranges",
    "ranges of ASNs",
    { name: z.string().optional(), rir_id: z.number().int().optional() },
    asnRangeCreate,
    asnRangeUpdate,
    ["name", "slug", "rir", "start", "end"],
  );
  crud(
    "ipam/route-targets",
    "route_target",
    "route_targets",
    "BGP route targets for VRFs",
    { name: z.string().optional(), tenant_id: z.number().int().optional() },
    routeTargetCreate,
    routeTargetUpdate,
