...
const CONSOLE_TYPE =
  "Console port type slug, e.g. 'rj-45', 'usb-a', 'usb-c', 'de-9', 'db-25'.";
const PORT_TYPE =
  "Port type slug, e.g. '8p8c', 'lc', 'lc-pc', 'sc', 'st', 'mpo', 'fc', 'splice'.";
const IFACE_TYPE =
  "Interface type slug, e.g. '1000base-t', '10gbase-x-sfpp', '25gbase-x-sfp28', '100gbase-x-qsfp28', 'virtual', 'lag'.";
const DEVICE_ID = z
  .number()
  .int()
  .describe(
...
const moduleCreate = {
  module_bay: z
    .number()
    .int()
    .describe("Module bay id this module installs into (required)."),
...
const moduleBayCreate = {
  device_id: DEVICE_ID,
  name: z.string().min(1).max(64).describe("Module bay name (required)."),
  label: z.string().max(64).optional(),
  position: z.string().max(30).optional().describe("Position identifier."),
  description: z.string().max(200).optional(),
  tags: TagSlugsSchema,
  custom_fields: CustomFieldsSchema,
};
...
const deviceBayCreate = {
  device_id: DEVICE_ID,
  name: z.string().min(1).max(64).describe("Device bay name (required)."),
  label: z.string().max(64).optional(),
  installed_device: z
    .number()
    .int()
    .optional()
    .describe("Child device id installed in this bay."),
  description: z.string().max(200).optional(),
  tags: TagSlugsSchema,
  custom_fields: CustomFieldsSchema,
};
...
const invItemCreate = {
  device_id: DEVICE_ID,
  name: z.string().min(1).max(64).describe("Inventory item name (required)."),
  label: z.string().max(64).optional(),
  ...
  part_id: z.string().max(50).optional(),
  serial: z.string().max(50).optional(),
  asset_tag: z.string().max(50).optional(),
  ...
  component_type: z
    .string()
    .optional()
    .describe("Optional linked component type, e.g. 'dcim.interface'."),
...
const consolePortCreate = {
  device_id: DEVICE_ID,
  name: z.string().min(1).max(64).describe("Port name (required)."),
  label: z.string().max(64).optional(),
  type: z.string().optional().describe(CONSOLE_TYPE),
...
const rearPortCreate = {
  device_id: DEVICE_ID,
  name: z.string().min(1).max(64).describe("Rear port name (required)."),
  ...
  positions: z
    ...
    .max(1024)
    .optional()
    .describe("Number of front ports mappable (default 1)."),
  mark_connected: z.boolean().optional(),
  description: z.string().max(200).optional(),
  tags: TagSlugsSchema,
  custom_fields: CustomFieldsSchema,
...
const frontPortCreate = {
  device_id: DEVICE_ID,
  name: z.string().min(1).max(64).describe("Front port name (required)."),
  type: z.string().describe(PORT_TYPE + " (required)."),
  rear_port: z
    .number()
    .int()
    .describe("Rear port id this front port maps to (required)."),
  rear_port_position: z
    .number()
    .int()
    .min(1)
    .optional()
    .describe("Position on the rear port (default 1)."),
  label: z.string().max(64).optional(),
  color: z.string().optional(),
  mark_connected: z.boolean().optional(),
  description: z.string().max(200).optional(),
...
const frontPortUpdate = {
  ...
  rear_port_position: z.number().int().min(1).optional(),
  label: z.string().max(64).optional(),
  color: z.string().optional(),
  mark_connected: z.boolean().optional(),
  description: z.string().max(200).optional(),
...
const macCreate = {
  ...
  assigned_object_type: z
    ...
    .optional()
    .describe("Set with assigned_object_id to attach to an interface."),
  assigned_object_id: z
    .number()
    .int()
    .optional()
    .describe("Interface / VM-interface id."),
  description: z.string().max(200).optional(),
  comments: z.string().optional(),
  tags: TagSlugsSchema,
  custom_fields: CustomFieldsSchema,
...
const tmplParent = {
  device_type: z
    ...
    .int()
    .optional()
    .describe("Device type id (set this OR module_type)."),
  module_type: z
    .number()
    .int()
    .optional()
    .describe("Module type id (set this OR device_type)."),
};
const consoleTmplCreate = {
  ...tmplParent,
  name: z.string().min(1).max(64).describe("Name (required)."),
  label: z.string().max(64).optional(),
  type: z.string().optional().describe(CONSOLE_TYPE),
  description: z.string().max(200).optional(),
};
const consoleTmplUpdate = {
...
const ifaceTmplCreate = {
  ...tmplParent,
  name: z.string().min(1).max(64).describe("Name (required)."),
  type: z.string().describe(IFACE_TYPE + " (required)."),
...
const frontTmplCreate = {
  ...tmplParent,
  name: z.string().min(1).max(64).describe("Name (required)."),
  type: z.string().describe(PORT_TYPE + " (required)."),
  rear_port: z.number().int().describe("Rear port TEMPLATE id (required)."),
  rear_port_position: z.number().int().min(1).optional(),
  color: z.string().optional(),
  label: z.string().max(64).optional(),
  description: z.string().max(200).optional(),
};
const frontTmplUpdate = {
  device_type: z.number().int().optional(),
  module_type: z.number().int().optional(),
  name: z.string().min(1).max(64).optional(),
  type: z.string().optional(),
...
const deviceBayTmplCreate = {
  device_type: z.number().int().describe("Device type id (required)."),
  name: z.string().min(1).max(64).describe("Name (required)."),
  label: z.string().max(64).optional(),
  description: z.string().max(200).optional(),
};
const deviceBayTmplUpdate = {
  device_type: z.number().int().optional(),
  name: z.string().min(1).max(64).optional(),
  label: z.string().max(64).optional(),
  description: z.string().max(200).optional(),
...
export function registerDcimComponents(server: McpServer): void {
  ...
  crud(
    ...
    "module_type",
    "module_types",
    "module models (line cards, etc.)",
    { manufacturer_id: z.number().int().optional(), model: z.string().optional() },
    moduleTypeCreate,
    moduleTypeUpdate,
    ["model", "manufacturer", "part_number"],
  );
  crud(
    "dcim/modules",
    "module",
    ...
    {
      device_id: z.number().int().optional(),
      module_bay_id: z.number().int().optional(),
      status: z.string().optional(),
    },
    moduleCreate,
    moduleUpdate,
    ["module_type", "module_bay", "status", "serial"],
  );
  crud(
    "dcim/module-bays",
    "module_bay",
    "module_bays",
    "bays that hold modules",
    dev,
    moduleBayCreate,
    moduleBayUpdate,
  ...
  crud(
    ...
    "device_bay",
    "device_bays",
    "bays that hold child devices",
    dev,
    deviceBayCreate,
    deviceBayUpdate,
    ["name", "device", "installed_device"],
  );
  crud(
  ...
  crud(
    ...
    "inventory_item",
    "inventory_items",
    "discrete inventory items on a device",
    {
      ...dev,
    ...
    invItemCreate,
    invItemUpdate,
    ["name", "device", "role", "manufacturer", "serial"],
  );
  crud(
    "dcim/console-ports",
    "console_port",
  ...
  crud(
    ...
    "rear_port",
    "rear_ports",
    "patch-panel rear ports",
    { ...dev, type: z.string().optional() },
    rearPortCreate,
    rearPortUpdate,
    ["name", "device", "type", "positions"],
  );
  crud(
    "dcim/front-ports",
    "front_port",
  ...
  crud(
    ...
    macCreate,
    macUpdate,
    ["mac_address", "assigned_object", "description"],
  );
  ...
  crud(
    ...
    ifaceTmplCreate,
    ifaceTmplUpdate,
    ["name", "device_type", "type", "mgmt_only"],
  );
  crud(
    ...
    rearTmplCreate,
    rearTmplUpdate,
    ["name", "device_type", "type", "positions"],
  );
  crud(
    ...
    frontTmplCreate,
    frontTmplUpdate,
    ["name", "device_type", "type", "rear_port"],
  );
  crud(
  ...
  crud(
    ...
    invItemTmplCreate,
    invItemTmplUpdate,
    ["name", "device_type", "role", "manufacturer"],
  );
}
