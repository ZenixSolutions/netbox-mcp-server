...
const regionCreate = {
  ...
  parent: z.number().int().optional().describe("Parent region id (regions are nestable)."),
...
/* site groups */
const siteGroupFilters = { name: z.string().optional(), slug: z.string().optional(), parent_id: z.number().int().optional() };
...
const rackTypeFilters = {
  manufacturer_id: z.number().int().optional(), model: z.string().optional(), slug: z.string().optional(),
};
const rackTypeCreate = {
  ...
  slug: z.string().min(1).max(100).regex(SLUG).describe("URL slug (required)."),
  form_factor: z.string().optional().describe("2-post-frame, 4-post-frame, 4-post-cabinet, wall-frame, wall-frame-vertical, wall-cabinet, wall-cabinet-vertical."),
  ...
  weight: z.number().optional(), max_weight: z.number().int().optional(),
...
const rackTypeUpdate = {
  ...
  weight: z.number().nullable().optional(), max_weight: z.number().int().nullable().optional(),
...
const rackReservationFilters = {
  rack_id: z.number().int().optional(), user_id: z.number().int().optional(), tenant_id: z.number().int().optional(),
};
const rackReservationCreate = {
  rack: z.number().int().describe("Rack id (required)."),
  units: z.array(z.number().int()).describe("Array of rack unit numbers to reserve (required), e.g. [1,2,3]."),
  ...
  comments: z.string().optional(), tags: TagSlugsSchema, custom_fields: CustomFieldsSchema,
};
...
export function registerDcimOrg(server: McpServer): void {
  ...
  crud("dcim/regions", "region", "regions", "geographic regions (nestable)", regionFilters, regionCreate, regionUpdate, ["name", "slug", "parent", "description"]);
  crud("dcim/site-groups", "site_group", "site_groups", "functional site groups (nestable)", siteGroupFilters, siteGroupCreate, siteGroupUpdate, ["name", "slug", "parent", "description"]);
  crud("dcim/rack-roles", "rack_role", "rack_roles", "rack roles", rackRoleFilters, rackRoleCreate, rackRoleUpdate, ["name", "slug", "color", "description"]);
  crud("dcim/rack-types", "rack_type", "rack_types", "predefined rack models", rackTypeFilters, rackTypeCreate, rackTypeUpdate, ["model", "slug", "manufacturer", "form_factor", "u_height", "width"]);
  crud("dcim/rack-reservations", "rack_reservation", "rack_reservations", "reserved units within a rack", rackReservationFilters, rackReservationCreate, rackReservationUpdate, ["rack", "units", "user", "tenant", "description"]);
}
