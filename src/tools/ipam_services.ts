...
const fhrpUpdate = {
  protocol: z.enum(FHRP_PROTO).optional(),
  group_id: z.number().int().optional(),
  name: z.string().max(100).optional(),
  auth_type: z.enum(["plaintext", "md5"]).nullable().optional(),
  auth_key: z.string().max(255).optional(),
  description: z.string().max(200).optional(),
  comments: z.string().optional(),
  tags: TagSlugsSchema,
...
const fhrpAsgnCreate = {
  ...
  interface_type: z
    ...
    .describe("Interface object type (required)."),
...
const serviceCreate = {
  parent_object_type: z
    .enum(["dcim.device", "virtualization.virtualmachine", "ipam.fhrpgroup"])
    .describe(
      "Parent object type (required). NetBox 4.3+ uses a generic parent, not device/virtual_machine.",
    ),
  ...
  ipaddresses: z
    .array(z.number().int())
    .optional()
    .describe("IP address ids this service is bound to."),
  description: z.string().max(200).optional(),
  comments: z.string().optional(),
  tags: TagSlugsSchema,
  custom_fields: CustomFieldsSchema,
...
const serviceUpdate = {
  ...
  protocol: z.enum(L4_PROTO).optional(),
...
const svcTmplCreate = {
  name: z.string().min(1).max(100).describe("Template name (required)."),
  protocol: z.enum(L4_PROTO).describe("L4 protocol (required)."),
  ports: PORTS.describe("Array of ports (required)."),
  description: z.string().max(200).optional(),
  comments: z.string().optional(),
  tags: TagSlugsSchema,
  custom_fields: CustomFieldsSchema,
...
const vlanXPolicyCreate = {
  name: z.string().min(1).max(100).describe("Policy name (required)."),
  description: z.string().max(200).optional(),
  comments: z.string().optional(),
};
const vlanXPolicyUpdate = {
  name: z.string().min(1).max(100).optional(),
  description: z.string().max(200).optional(),
  comments: z.string().optional(),
};

const vlanXRuleCreate = {
  policy: z.number().int().describe("VLAN translation policy id (required)."),
  local_vid: z.number().int().min(1).max(4094).describe("Local VID (required)."),
  remote_vid: z.number().int().min(1).max(4094).describe("Remote VID (required)."),
  description: z.string().max(200).optional(),
};
const vlanXRuleUpdate = {
  policy: z.number().int().optional(),
  local_vid: z.number().int().min(1).max(4094).optional(),
  remote_vid: z.number().int().min(1).max(4094).optional(),
  description: z.string().max(200).optional(),
};
...
export function registerIpamServices(server: McpServer): void {
  ...
  crud(
    ...
    "fhrp_group",
    "fhrp_groups",
    "First Hop Redundancy Protocol groups (VRRP/HSRP/etc.)",
    { protocol: z.string().optional(), group_id: z.number().int().optional() },
    fhrpCreate,
    fhrpUpdate,
    ["protocol", "group_id", "name", "auth_type"],
  );
  crud(
    "ipam/fhrp-group-assignments",
    "fhrp_group_assignment",
    "fhrp_group_assignments",
    "FHRP group to interface assignments",
    { group_id: z.number().int().optional() },
    fhrpAsgnCreate,
    fhrpAsgnUpdate,
    ["group", "interface_type", "priority"],
  );
  crud(
    "ipam/services",
    "service",
    "services",
    "L4 services on devices/VMs",
    { name: z.string().optional(), protocol: z.string().optional() },
    serviceCreate,
    serviceUpdate,
    ["name", "protocol", "ports", "parent"],
  );
  crud(
